  ...
  dots.forEach((out, i) => {
    ...
    $("#learningOutcomes").append(precede).append(
      ...
      .append(
        $('<div class="form-group row justify-content-center">')
          .append(
            $('<label class="col-sm-10">').text("Instructors will...").append('<span class="text-danger">*</span> <span class="text-info fa fa-question-circle" data-toggle="tooltip" data-placement="bottom" title="Write the teaching methods that instructors will employ to achieve this learning outcome. You should describe the teaching methods using action verbs in a single sentence that continues the phrase \'Instructors will\'"></span>')
          ).append(
            $('<textarea rows="2" class="col-sm-10 form-control outcomesTeaching" value="" name="outcomes['+outs[i]+'][teaching]">')
        )
      )
    )
